...
2. [Claude Code configuration (.claude/)](#2-claude-code-configuration-claude)
   ...
   - [CLAUDE.md — session entry point](#22b-claudemd--session-entry-point)
   - [Hooks](#23-hooks)
   - [Skills](#24-skills)
...
**Secret required:** `PROJECT_PAT` *(org-level — inherited automatically)*  
...
**Secret required:** `PROJECT_PAT` *(org-level — inherited automatically)* (optional — for status check only)
...
3. Saves `ORG`, `REPO`, `PROJECT_NUMBER` to `.claude-github-config.json` for future re-runs and `--ci` mode.
4. Walks every file in `template/` and for each:
...
| Any file listed in `.claude-github-config-ignore` | Protected from being **overwritten** — initial creation always proceeds even if the file is listed |
...
| `{{PROJECT_NUMBER}}` | GitHub Project v2 board number | `42` |
