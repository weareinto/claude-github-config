...
- A member of the `{{ORG}}` GitHub organization
- A collaborator on `{{ORG}}/{{REPO}}`
- A member of the `{{ORG}}/{{PROJECT_NUMBER}}` GitHub Project (https://github.com/orgs/{{ORG}}/projects/{{PROJECT_NUMBER}})
...
Pick the right template at https://github.com/{{ORG}}/{{REPO}}/issues/new/choose:
...
2. **The ticket must be in Ready status.** Check the issue's project card on [Project board #{{PROJECT_NUMBER}}](https://github.com/orgs/{{ORG}}/projects/{{PROJECT_NUMBER}}):
...
- **Humans:** drag the card in the [Project board UI](https://github.com/orgs/{{ORG}}/projects/{{PROJECT_NUMBER}}).
...
/
├── README.md
├── CLAUDE.md              # Entry for Claude Code (imports CONTRIBUTING + PROJECT)
├── CONTRIBUTING.md        # This file — workflow source of truth
├── CHANGELOG.md           # Generated by towncrier — do not edit outside a release PR
├── changelog/             # Per-PR fragment files; consumed at release time
├── towncrier.toml         # Towncrier config
├── src/                   # Source code
├── .claude/               # Claude Code harness config + skills + hooks
├── .github/               # Issue/PR templates, workflows, helper scripts
└── doc/
    ├── PROJECT.md         # Product context (what this repo is)
